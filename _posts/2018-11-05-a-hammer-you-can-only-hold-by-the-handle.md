...
No `thing` there, it went out of scope on line 10. Importantly, `thing` goes out of scope before its ownership is transfered, so Rust _drops_ it: the compiler inserts code to clean up all resources associated with `thing` and frees its memory. We can hook into this mechanism by providing an implementation of the special `Drop` trait:
...
3. we don't want to forget to release a resource when we're done: we ensure we tell the driver they can leave.<br/>
...
Here's the relevant code: [`std::rc::Rc`](https://doc.rust-lang.org/src/alloc/rc.rs.html#802-847).
...
[`std::sync::MutexGuard`](https://doc.rust-lang.org/src/std/sync/mutex.rs.html#452-460).
...
<i>Thanks for reading! You can send me notes and questions [on twitter](https://twitter.com/utaal). There's a [lobste.rs](https://lobste.rs/s/5nmyze/hammer_you_can_only_hold_by_handle) and a [reddit](https://www.reddit.com/r/rust/comments/9uccla/a_hammer_you_can_only_hold_by_the_handle/) post for comments.</i>
